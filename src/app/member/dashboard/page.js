      ...
      <div className="bg-gradient-to-r from-[#03215F] to-[#03215F] rounded-xl md:rounded-2xl p-4 md:p-8 text-white relative overflow-hidden mx-2 md:mx-0">
        ...
        <div className="relative">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-2 md:gap-3">
            ...
            <div className={`px-3 py-1.5 backdrop-blur-sm rounded-full text-xs md:text-sm flex items-center justify-center md:justify-start ${displayIsActive
              ? 'bg-green-500/20 text-white'
              : 'bg-[#b8352d]/20 text-white'
              }`}>
              <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full mr-1.5 md:mr-2 ${displayIsActive ? 'bg-green-500' : 'bg-[#b8352d]'
                }`}></div>
            ...
            {isPremiumMember && (() => {
              ...
              return (
                <div className={`px-3 py-1.5 backdrop-blur-sm rounded-full text-xs md:text-sm flex items-center justify-center md:justify-start ${timeInfo.isExpired ? 'bg-[#b8352d]/20 text-white' : 'bg-green-500/20 text-white'
                  }`}>
                  <Clock className="w-3 h-3 md:w-4 md:h-4 mr-1.5 md:mr-2" />
                  <span className="font-medium">{timeInfo.label}</span>
                </div>
              );
            })()}
      ...
      <div className='px-2 md:px-0'>
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 lg:gap-4">
          {dashboardStats.map((stat, index) => (
            <div
              key={index}
              className={`bg-white rounded-lg shadow p-3 md:p-4 lg:p-5 border ${stat.color === 'amber'
                ? 'border-[#ECCF0F]/20'
                : 'border-gray-200'
                }`}
            >
              <div className="flex items-center justify-between mb-2">
                <div className={`p-1.5 md:p-2 rounded-lg flex-shrink-0 ${stat.color === 'blue' ? 'bg-[#9cc2ed]' :
                  stat.color === 'green' ? 'bg-[#AE9B66]' :
                    stat.color === 'purple' ? 'bg-[#03215F]' :
                      stat.color === 'amber' ? 'bg-[#ECCF0F]' :
                        'bg-gray-100'
                  }`}>
                  <stat.icon className={`w-4 h-4 md:w-5 md:h-5 ${stat.color === 'blue' ? 'text-[#03215F]' :
                ...
                <div className={`text-xs font-semibold truncate ml-1 ${stat.color === 'blue' ? 'text-[#03215F]' :
        ...
        <div className="grid lg:grid-cols-3 mt-4 md:mt-5 gap-3 md:gap-4 lg:gap-6">
          ...
          <div className="lg:col-span-2 space-y-3 md:space-y-4 lg:space-y-6">
            ...
            {/* Membership Time Strip — for paid members only */}
            {isPremiumMember && (() => {
              const expirySource = user?.membership_expiry_date || currentSubscription?.expires_at;
              const timeInfo = getTimeRemaining(expirySource);
              if (!timeInfo) return null;
                ...
                return (
                  <div className="mt-3 p-4 rounded-xl bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <div className="p-2 bg-[#b8352d]/10 rounded-lg mt-0.5">
                        <AlertCircle className="w-5 h-5 text-[#b8352d]" />
                      </div>
                      <div>
                    ...
                    <Link href="/member/dashboard/subscriptions">
                      <button className="px-5 py-2.5 bg-[#b8352d] text-white rounded-lg font-semibold hover:bg-[#a02e27] transition-colors text-sm whitespace-nowrap flex items-center gap-2">
              ...
              return (
                <div className="mt-3 p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 flex items-center gap-3">
            ...
            <div className="bg-gradient-to-r from-[#9cc2ed]/10 to-[#9cc2ed]/10 rounded-xl p-4 md:p-5 lg:p-6 border border-[#9cc2ed]/20">
              ...
              <div className="space-y-3">
                <div className={`p-3 rounded-lg ${displayIsActive
                  ? 'bg-[#AE9B66]/20 border border-[#AE9B66]/30'
                  : 'bg-[#b8352d]/20 border border-[#b8352d]/30'
                  }`}>
                  <div className="flex items-center">
                    <div className={`w-2 h-2 rounded-full mr-2 ${displayIsActive ? 'bg-[#AE9B66]' : 'bg-[#b8352d]'
                      }`}></div>
                ...
                {user?.membership_expiry_date && (() => {
                  ...
                  const timeInfo = getTimeRemaining(expirySource);
                  return (
                    <div className="flex flex-col p-3 bg-white/50 rounded-lg gap-2">
                      ...
                      {timeInfo && (
                        <div className={`text-xs font-semibold px-2 py-1 rounded-md inline-flex items-center gap-1 self-start ${timeInfo.isExpired ? 'bg-[#b8352d]/10 text-[#b8352d]' : 'bg-emerald-100 text-emerald-700'
                          }`}>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-5 lg:p-6 border border-gray-200">
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                {quickActions.map((action, index) => (
                  ...
                  >
                    <div className={`mb-2 md:mb-3 p-2 md:p-2.5 rounded-lg inline-flex ${action.color.includes('[#03215F]') ? 'bg-[#03215F]/80 text-[#03215F]' :
                    ...
                    <div className="mt-2">
                      <div className={`text-xs md:text-sm font-medium flex items-center ${action.color.includes('[#03215F]') ? 'text-[#03215F]' :
                ...
                <div className="space-y-2 md:space-y-3">
                  {recentActivities.slice(0, isMobile ? 3 : 5).map((activity) => (
                    <div key={activity.id} className="flex items-start gap-2">
                      <div className={`mt-0.5 p-1 rounded-full flex-shrink-0 ${activity.status === 'success'
                        ? 'bg-[#AE9B66]'
