...
export default function AdminCommitteeSectionsPage() {
	...
	const loadPages = async () => {
		...
		try {
			...
			console.error(e);
			toast.error("Failed to load committee sections");
		} finally {
			setLoading(false);
		}
	};
	...
	const openEditModal = (page) => {
		...
		setForm({
			...
			image_alignment: page.image_alignment || "left",
			button_label: page.button_label || "",
			button_url: page.button_url || "",
			show_button: !!page.show_button,
			...
			<AnimatePresence>
				{showCreateModal && (
					<Modal
						...
						title={editing ? "Edit Committee Section" : "Create New Section"}
						size="xl"
					>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									<div>
										...
										<select
											...
											onChange={(e) =>
												setForm((prev) => ({ ...prev, committee_id: e.target.value }))
											}
											required
								...
								<div className="mt-6 space-y-4">
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										<div>
											...
											<p className="mt-1 text-xs text-gray-500">
												Optional image shown alongside this section content.
											</p>
										</div>
										...
										<div>
											...
											<select
												value={form.image_alignment}
												onChange={(e) =>
													setForm((prev) => ({ ...prev, image_alignment: e.target.value }))
												}
												className="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#03215F] focus:border-transparent"
											>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										<div>
											...
											<input
												...
												onChange={(e) =>
													setForm((prev) => ({ ...prev, button_label: e.target.value }))
												}
												className="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#03215F] focus:border-transparent"
										...
										<div>
											...
											<input
												...
												onChange={(e) =>
													setForm((prev) => ({ ...prev, button_url: e.target.value }))
												}
												className="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#03215F] focus:border-transparent"
									...
									<div className="flex items-center gap-2 p-3 bg-white border border-gray-200 rounded-xl">
										<input
											...
											onChange={(e) =>
												setForm((prev) => ({ ...prev, show_button: e.target.checked }))
											}
											className="w-4 h-4 text-[#03215F] rounded focus:ring-[#03215F]"
										/>
										<label htmlFor="show_button" className="text-sm text-gray-700">
											Show call-to-action button for this section
										</label>
									</div>
								</div>

								<div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
									<p className="text-sm text-blue-700flex items-start gap-2">
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-gray-600 mt-1">
									Manage content sections for each committee
								</p>
							</div>
						</div>
					</div>
				...
				>
					{loading ? (
						<div className="flex flex-col items-center justify-center py-20">
							...
							<p className="text-gray-600">Loading committee sections...</p>
						</div>
					) : pages.length === 0 ? (
						<div className="flex flex-col items-center justify-center py-20">
